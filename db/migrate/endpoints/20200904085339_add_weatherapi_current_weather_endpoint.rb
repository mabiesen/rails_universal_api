class AddWeatherapiCurrentWeatherEndpoint < ActiveRecord::Migration[5.2]
  def up
  ...
  def down
    Endpoint.where(name: 'get_current_weather')
            .where(client_tag: 'weatherapi').first.try(:destroy)
  end
end
